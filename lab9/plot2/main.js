...
let dataset = [{
    ...
    timeOnline: 9.88,
    wordsWritten: 273,
    timeOnClasswork: 2.07,
    timeOffline: 14.12,
    date: "11/05/2025"
},{
    timeOnline: 9.47,
    wordsWritten: 0,
    timeOnClasswork: 0.82,
    timeOffline: 14.53,
    date: "11/06/2025"
},{
    timeOnline: 7.35,
    ...
    timeOnline: 10.95,
    wordsWritten: 374,
    timeOnClasswork: 2.35,
    timeOffline: 13.05,
    date: "11/08/2025"
},{
    timeOnline: 11.9,
    wordsWritten: 0,
    timeOnClasswork: 2.15,
    timeOffline: 12.1,
    date: "11/09/2025"
},{
    timeOnline: 9.33,
    wordsWritten: 516,
    timeOnClasswork: 1.18,
    timeOffline: 14.66,
    date: "11/10/2025"
}];

/*filters data to only display those points where timeOnClasswork > 0 */
let filteredData = dataset.filter(function(value){
    return value.timeOnClasswork != 0;
})

/*these two functions allowed for significant simplification of the code——now they track time offline rather than time online*/
let minTimeOffline = d3.min(filteredData, function(value){
    return value.timeOffline;
});

let maxTimeOffline = d3.max(filteredData, function(value){
    return value.timeOffline;
})
...
circles.attr("r", 10)
/*sets X value to reflect timeOnClasswork parameter from array dataset, each param multiplied by 200 to better fill the whole scale of the plot */
    .attr("cx", function (filteredData) {
        return (sideMargin + (filteredData.timeOnClasswork * 200));
    })
/*sets Y value to reflect wordsWritten parameter from array dataset
    ...
    .attr("cy", function (filteredData) {
        return ((svgHeight - bottomMargin) - filteredData.wordsWritten);
    })
...
let rScale = d3.scaleSqrt()
    .domain([minTimeOffline, maxTimeOffline])
    .range([minTimeOffline, maxTimeOffline]) 

circles.attr("r", function(filteredData){
    return rScale(filteredData.timeOnline);
})

/*these two variables are refs for the x + y coords of the key circles */
let keyXCoord = (sideMargin * 2) - 3;
...
let keyData = [{
    xValue: keyXCoord,
    yValue: keyYCoord,
    rValue: maxTimeOffline
}, {
    xValue: keyXCoord,
    yValue: (keyYCoord - (bottomMargin * 2)),
    rValue: ((minTimeOffline + maxTimeOffline) / 2)
}, {
    xValue: keyXCoord,
    yValue: (keyYCoord - (bottomMargin * 3.5)),
    rValue: minTimeOffline
}];
...
for (let i = 0; i < 3; i++){ //adds params to keyLabel and draws them on the canvas using the equivalent indexes in keyData as refs
    keyCirclesLabel[i] = svg.append("text")
        .attr("x", ((sideMargin * 2 ) + 18))
        .attr("y", ((bottomMargin * 2) + keyData[i].yValue))
        .attr("text-anchor", "start")
        .attr("alignment-baseline", "middle")
        .text(String(keyData[i].rValue.toFixed(3)) + " hrs")
}
...
/*these two functions allowed for significant simplification of the code——now they track time offline rather than time online*/
let minTimeOffline = d3.min(dataset, function(value){
    return value.timeOffline;
});

let maxTimeOffline = d3.max(dataset, function(value){
    return value.timeOffline;
})
...
/*sets Y value to reflect wordsWritten parameter from array dataset
    ...
    .attr("fill", function(dataset){
        if (dataset.timeOnline < 10){
            return "green";
        }
        else if (dataset.timeOnline >= 10 && dataset.timeOnline <= 14){
            return "blue";
        }
        else {
            return "red";
